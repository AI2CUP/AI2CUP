<template>
  <div class="bg-emerald-50 rounded-xl p-6 border border-emerald-100">
    ...
    <div class="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 mb-6">
      <div class="text-center">
        <div class="text-4xl font-bold text-gray-900 mb-1">
          ${{ formatPrice(result.predicted_price_usd) }}
        </div>
    ...
    <div class="bg-white rounded-lg p-4 text-sm text-gray-600 grid grid-cols-2 md:grid-cols-4 gap-4 border border-emerald-50">
      ...
      <div>
        <span class="block text-gray-400 text-xs mb-1">ECX Grade</span>
        <span class="font-medium">{{ result.inputs.ecx_grade }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Processing</span>
        <span class="font-medium">{{ result.inputs.processing }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Exporter Type</span>
        <span class="font-medium">{{ result.inputs.exporter_type }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Year</span>
        <span class="font-medium">{{ result.inputs.year }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Month</span>
        <span class="font-medium">{{ result.inputs.month }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Week</span>
        <span class="font-medium">{{ result.inputs.week || '—' }}</span>
      </div>
      <div>
        <span class="block text-gray-400 text-xs mb-1">Model</span>
        <span class="font-medium truncate" :title="result.model_info">{{ result.model_info.split('(')[0] }}</span>
      </div>
    </div>
  </div>
</template>
